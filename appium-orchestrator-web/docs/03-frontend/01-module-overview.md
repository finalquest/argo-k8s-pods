...
export async function getFeatureContent(branch, client, feature) {
  try {
    const response = await fetch(`/api/features/${branch}/${client}/${feature}`);
    if (!response.ok) {
      throw new Error(`Error ${response.status}: ${response.statusText}`);
    }
...
export function initializeSocketListeners(socket) {
  socket.on('job_started', (data) => {
    ...
    if (window.progressIndicatorManager && data.jobId) {
      ...
      if (data.featureName) {
        window.progressIndicatorManager.setJobFeature(data.jobId.toString(), data.featureName);
        
        const testFileName = data.featureName.endsWith('.feature') ? 
          data.featureName : `${data.featureName}.feature`;
...
function initializeAppControls(socket) {
  ...
  // Botones de acción
  document.getElementById('fetch-features-btn').addEventListener('click', () => {
    handleFetchFeatures(socket);
  });
  
  document.getElementById('prepare-workspace-btn').addEventListener('click', () => {
