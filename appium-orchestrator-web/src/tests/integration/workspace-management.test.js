...
describe('Workspace Management Integration Tests', () => {
  ...
  describe('Workspace Creation and Management', () => {
    test('should create workspace for branch', async () => {
      ...
      if (result.success) {
        ...
        // Cleanup
        fs.rmSync(path.join(testWorkspacesRoot, branchName), {
          recursive: true,
          force: true,
        });
      }
  ...
  describe('Git Operations in Workspace', () => {
    ...
    afterEach(async () => {
      ...
      if (fs.existsSync(path.join(testWorkspacesRoot, testBranch))) {
        fs.rmSync(path.join(testWorkspacesRoot, testBranch), {
          recursive: true,
          force: true,
        });
      }
  ...
  describe('Workspace File Operations', () => {
    ...
    beforeEach(async () => {
      ...
      fs.writeFileSync(
        path.join(featuresDir, 'test1.feature'),
        'Feature: Test 1',
      );
      fs.writeFileSync(
        path.join(featuresDir, 'test2.feature'),
        'Feature: Test 2',
      );
    });
    ...
    test('should write feature content', () => {
      ...
      if (typeof workspaceManager.saveFeatureContent === 'function') {
        ...
        const result = workspaceManager.saveFeatureContent(
          featurePath,
          content,
        );
    ...
    test('should handle workspace cleanup', async () => {
      ...
      expect(() => {
        if (fs.existsSync(path.join(testWorkspacesRoot, branch))) {
          fs.rmSync(path.join(testWorkspacesRoot, branch), {
            recursive: true,
            force: true,
          });
        }
  ...
  describe('Workspace Integration with Other Modules', () => {
    test('should integrate with branch manager', () => {
      ...
      expect(workspacePath).toContain(
        configManager.get('PERSISTENT_WORKSPACES_ROOT'),
      );
    });
    ...
    test('should handle git operations integration', async () => {
      ...
      const authUrl = gitOps.getAuthenticatedUrl(
