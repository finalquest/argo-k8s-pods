...
describe('createWorker Function ReferenceError Prevention', () => {
  ...
  describe('Parameter Access Patterns', () => {
    test('should access persistentWorkspace parameter directly, not through job object', () => {
      ...
      const mockCreateWorker = jest.fn((branch, client, apkIdentifier, apkSourceType, deviceSerial, persistentWorkspace = false) => {
      ...
      expect(() => {
        mockCreateWorker('test-branch', { id: 'client1' }, 'test-apk', 'registry', 'emulator-5554', false);
      }).not.toThrow();

      expect(consoleSpy.log).toHaveBeenCalledWith('[SERVER] Usando workspace temporal');
    });
    ...
    test('should handle persistentWorkspace parameter correctly when true', () => {
      ...
      expect(() => {
        mockCreateWorker('test-branch', { id: 'client1' }, 'test-apk', 'registry', 'emulator-5554', true);
      }).not.toThrow();

      expect(consoleSpy.log).toHaveBeenCalledWith('[SERVER] Usando workspace persistente');
    });
  ...
  describe('Regression Test - ReferenceError Prevention', () => {
    test('should demonstrate the old bug pattern (which would throw ReferenceError)', () => {
      ...
      const buggyCreateWorker = jest.fn((branch, client, apkIdentifier, apkSourceType, deviceSerial, persistentWorkspace = false) => {
        ...
        try {
          ...
          if (job && job.persistentWorkspace && process.env.PERSISTENT_WORKSPACES_ROOT) {
            return { type: 'persistent', branch };
          } else {
            return { type: 'temporary', branch };
          }
          ...
          if (error instanceof ReferenceError && error.message.includes('job is not defined')) {
      ...
      expect(() => {
        buggyCreateWorker('test-branch', { id: 'client1' }, 'test-apk', 'registry', 'emulator-5554', true);
    ...
    test('should demonstrate the correct pattern (which should not throw)', () => {
      ...
      const fixedCreateWorker = jest.fn((branch, client, apkIdentifier, apkSourceType, deviceSerial, persistentWorkspace = false) => {
      ...
      expect(() => {
        fixedCreateWorker('test-branch', { id: 'client1' }, 'test-apk', 'registry', 'emulator-5554', true);
  ...
  describe('Default Parameter Behavior', () => {
    test('should handle default persistentWorkspace parameter correctly', () => {
      ...
      expect(() => {
        mockCreateWorker('test-branch', { id: 'client1' }, 'test-apk', 'registry', 'emulator-5554');
      }).not.toThrow();
    });
  });
});