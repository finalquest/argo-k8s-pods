...
describe('Core API Functionality', () => {
  ...
  describe('Workspace Management', () => {
    ...
    test('should handle temporary workspace path generation', () => {
      ...
      const tempWorkspacePath = path.join(
        os.tmpdir(),
  ...
  describe('History and Reports', () => {
    ...
    test('should handle history filtering by branch', () => {
      ...
      expect(filteredHistory.every((item) => item.branch === 'main')).toBe(
        true,
      );

      // Filter by branch 'develop'
      const developHistory = mockHistory.filter(
        (item) => item.branch === 'develop',
      );
      expect(developHistory.length).toBe(1);
      expect(developHistory.every((item) => item.branch === 'develop')).toBe(
        true,
      );
    });
