...
describe('Socket.io Communication', () => {
  ...
  describe('Room Management', () => {
    ...
    test('should broadcast to specific rooms', () => {
      ...
      mockBroadcast('job-1', 'job-update', {
        jobId: 'job-1',
        status: 'completed',
      });
      ...
      expect(broadcastMessages.every((msg) => msg.event === 'job-update')).toBe(
        true,
      );
    });
  ...
  describe('Error Handling', () => {
    ...
    test('should handle invalid event data', () => {
      ...
      expect(
        validationErrors.every((error) => error === 'Invalid job data'),
      ).toBe(true);
    });
  });
