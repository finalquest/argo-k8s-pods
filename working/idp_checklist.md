...
4. [x] Sincronizar el nuevo Application en Argo CD y validar pods listos + NodePort accesible para que Nginx Proxy Manager pueda apuntar a `auth.finalq.xyz`.
5. [x] Configurar Keycloak: realm `homelab`, usuario local de emergencia, IdP de Google y clientes `argocd`/`grafana`.
...
7. [x] Documentar y automatizar backups (`scripts/backup-keycloak-db.sh`, `scripts/export-keycloak-realm.sh`) y definir plan para integrar servicios externos (Vaultwarden, Pi-hole, Home Assistant) mediante OIDC.
